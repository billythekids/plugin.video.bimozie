...
def show_site_category():
    ...
    searchable = query.get('searchable')
...
def show_site_subcategory():
    ...
    query = json.loads(plugin.args['query'][0])
    instance, module, class_name = load_plugin(query)
    xbmcplugin.setPluginCategory(plugin.handle, '{} / {}'.format(class_name, helper.text_encode(query.get('name'))))

    for cat in query.get('subcategory'):
        list_item = xbmcgui.ListItem(label=cat.get('title'))
        list_item.addContextMenuItems(globalContextMenu())
        url = plugin.url_for(show_movies, query=json.dumps({
            'url': cat.get('link'), 'page': 1, 'name': '{} - {}'.format(
                helper.text_encode(query.get('name')), helper.text_encode(cat.get('title'))),
            'module': module, 'className': class_name
        }))
        xbmcplugin.addDirectoryItem(plugin.handle, url, list_item, isFolder=True)
    xbmcplugin.endOfDirectory(plugin.handle)
...
def show_fshare_folder():
    ...
    movie_item = query.get('movie_item')
    item = query.get('item')
    page = query.get('page') if query.get('page') else 1
    code = query.get('code')
...
def show_last_watched():
    items = helper.get_history_movie('watched.json')
    if items:
...
def clear_last_watched():
    helper.clear_history_movie('watched.json')
    return
...
def play_with_fshare_code():
    ...
    # Support to save search history
    items = helper.get_history_movie('fshare-watched.json')
    for item in items.values():
...
def clear_with_fshare_code():
    helper.clear_history_movie('fshare-watched.json')
