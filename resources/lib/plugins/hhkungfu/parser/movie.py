...
class Parser:
    ...
    def get_link(self, response, domain, movie_url):
        ...
        jobs = [{
                    'url': f"{domain}/wp-content/themes/halimmovies/player.php?episode_slug={EPI}&server_id={SVID}&subsv_id={i.get('data-subsv-id')}&post_id={PID}&nonce=&custom_var=",
                    'parser': Parser.extract_link} for i in servers]
        h = {
