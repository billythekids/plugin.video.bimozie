# coding=utf-8
from bs4 import BeautifulSoup


class Parser:
    def get(self):
        category = [
            {
                'title': '4K',
                'link': 'http://www.hdvietnam.com/forums/4k.337/'
            },
            {
                'title': 'Fshare.vn',
                'link': 'http://www.hdvietnam.com/forums/fshare-vn.33/'
            },
            {
                'title': 'Fshare.vn - WEB-DL, HDTV',
                'link': 'http://www.hdvietnam.com/forums/web-dl-hdtv.271/'
            },
            {
                'title': 'Fshare.vn - Bluray Remux',
                'link': 'http://www.hdvietnam.com/forums/bluray-remux.324/'
            },
            {
                'title': 'Fshare.vn -HD, SD',
                'link': 'http://www.hdvietnam.com/forums/mhd-sd.77/'
            },
            {
                'title': 'Fshare.vn -Bluray nguyên gốc',
                'link': 'http://www.hdvietnam.com/forums/bluray-nguyen-goc.78/'
            },
            {
                'title': 'Phim có audio Việt',
                'link': 'http://www.hdvietnam.com/forums/phim-co-audio-viet.265/'
            },
            {
                'title': 'Phim bộ - Series',
                'link': 'http://www.hdvietnam.com/forums/phim-bo-series.57/'
            },
            {
                'title': 'Phim bộ - mHD, SD',
                'link': 'http://www.hdvietnam.com/forums/mhd-sd.104/'
            },
            {
                'title': 'Phim hoạt hình',
                'link': 'http://www.hdvietnam.com/forums/phim-hoat-hinh.123/'
            },
            {
                'title': 'Phim hoạt hình - mHD, SD',
                'link': 'http://www.hdvietnam.com/forums/mhd-sd.124/'
            },
            {
                'title': 'Phim tài liệu - Documentaries',
                'link': 'http://www.hdvietnam.com/forums/phim-tai-lieu-documentaries.116/'
            },
            {
                'title': '3D',
                'link': 'http://www.hdvietnam.com/forums/3d.110/'
            },
            {
                'title': 'Phim cho iOS/Android',
                'link': 'http://www.hdvietnam.com/forums/phim-cho-ios-android.157/'
            },
        ]

        return category
